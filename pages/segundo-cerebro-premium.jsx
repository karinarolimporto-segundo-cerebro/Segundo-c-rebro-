...
function CalendarioInteligente({ eventos, setEventos }) {
  ...
  function toggleEvento(chave, id) {
    setEventos(e => ({
      ...e,
      [chave]: e[chave].map(x => x.id === id ? { ...x, feito: !x.feito } : x),
    }));
  }
  ...
  function removeEvento(chave, id) {
    setEventos(e => ({
      ...e,
      [chave]: e[chave].filter(x => x.id !== id),
    }));
  }
      ...
      <CardBase color={COLORS.amarelo} icon="📆" title={`${diaSelected} de ${MESES[mes]}`} minH={340}>
        ...
        <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', alignItems: 'flex-end' }}>
          ...
          <select value={corEvento} onChange={e => setCorEvento(e.target.value)} style={{
            ...
            <option value={COLORS.rosa} style={{ background: COLORS.rosa }}>🌸 Rosa</option>
            <option value={COLORS.azul} style={{ background: COLORS.azul }}>💙 Azul</option>
            <option value={COLORS.menta} style={{ background: COLORS.menta }}>🌿 Menta</option>
            <option value={COLORS.amarelo} style={{ background: COLORS.amarelo }}>⭐ Amarelo</option>
          </select>
          ...
          body: JSON.stringify({
            ...
            messages: [{
              ...
              content: [
                ...
                {
                  type: 'text',
                  text: `Analize esta foto de medicamento/suplemento e extraia APENAS em formato JSON:
{
...
Responda APENAS com JSON, nada mais.`,
  ...
  function marcarTomado(id) {
    setMeds(m => m.map(x => x.id === id ? { ...x, qtdUsada: x.qtdUsada + 1 } : x));
  }

  function remove(id) { setMeds(m => m.filter(x => x.id !== id)); }
...
function PastaModelos({ modelos, setModelos }) {
  ...
  function remove(id) { setModelos(m => m.filter(x => x.id !== id)); }
...
const STORAGE_KEY = 'segundo-cerebro-premium-v2';
...
export default function App() {
  const [estado, setEstado] = useState(() => {
    try {
      ...
      return { eventos: {}, meds: [], financas: [], modelos: [] };
    }
  });
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        ...
        <div style={{
          ...
          <CalendarioInteligente
            ...
            setEventos={e => setEstado({ ...estado, eventos: e })}
          />
          <MedicamentosComFoto
            meds={estado.meds}
            setMeds={m => setEstado({ ...estado, meds: m })}
          />
          <FinancasFamiliares
            financas={estado.financas}
            setFinancas={f => setEstado({ ...estado, financas: f })}
          />
          <PastaModelos
            modelos={estado.modelos}
            setModelos={m => setEstado({ ...estado, modelos: m })}
          />
        </div>
        ...
        <div style={{
          ...
          💾 Salvo automaticamente · 🧠 Segundo Cérebro Premium · Feito com 💜
        </div>
      </div>
    </div>
  );
